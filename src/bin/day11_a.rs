...
struct Monkey {
    ...
    op: Box<dyn Fn(u64) -> (u64, usize)> // u64: new, usize: monkey index
}
...
impl MonkeyBuilder {
    ...
    fn build(self) -> Monkey {
        ...
        Monkey {
            ...
            op: match op {
                MonkeyOperations::Add(x) => Box::new(move |old| {let new = (old + x) / 3; if new % test == 0 {(new, left)} else {(new, right)}}),
                MonkeyOperations::Multiply(x) => Box::new(move |old| {let new = (old * x) / 3; if new % test == 0 {(new, left)} else {(new, right)}}),
                MonkeyOperations::Square => Box::new(move |old| {let new = (old * old) / 3; if new % test == 0 {(new, left)} else {(new, right)}})
            },
        }
    }
}
...
fn main() -> Result<(), Box<dyn Error>> {
        ...
        .for_each(|x| {
            ...
            match captures.as_ref().map(|x| x.as_slice()) {
                ...
                Some(["Starting items", x]) => {
                    mb.starting_items = Some(x.split(", ").map(|x| x.parse::<u64>().unwrap()).collect());
                }
                Some(["Operation", x]) => {
    ...
    for _ in 0..20 {
        for monkey in &monkeys {
            *monkey.inspections.borrow_mut() += monkey.items.borrow().len();
